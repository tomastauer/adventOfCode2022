...
export default class Day17 implements Solution {
	...
	solvePart1(input: string[]) {
		...
		for(let i = 0; i < 2022; i++) {
			...
			while(true) {
				...
				if(jet === '>') {
					if(filledPositions.some(f => ['#', '|'].includes(game[f.y][f.x+1]))) {
						...
						filledPositions = filledPositions.map(({x,y})=>({x:x+1,y}));
					}
				} else {
					if(filledPositions.some(f => ['#', '|'].includes(game[f.y][f.x-1]))) {
				...
				if(filledPositions.some(f => ['#', '-'].includes(game[f.y-1][f.x]))) {
					// cannot move down, finished
					filledPositions.forEach(({x, y})=> {
						game[y][x] = '#';
					});

					highest = Math.max(highest -1, ...filledPositions.map(f => f.y)) + 1;
					break;
				} else {
					filledPositions.forEach(({x, y})=> {
						game[y][x] = '.';
					...
					filledPositions = filledPositions.map(({x,y})=>({x,y: y-1}));
				}
			}
		}
	...
	solvePart2(input: string[]) {
		...
		const pattern = game.slice(first, first + period).map(g => g.join(''));
		...
		for(let i = 0; i < 100000; i++) {
			...
			if(game[first] && game[first].join('') === pattern[0] && stepsBeforePeriod === 0) {
				stepsBeforePeriod = i;
			}

			if(game.length > first + period && stepsForPeriod === 0 && game.slice(first, first + period).map(g => g.join('')).join(',') === pattern.join(',')) {
				// We have full period
		...
		const runFor = stepsBeforePeriod + stepsAfter-Math.floor(stepsAfter / (stepsForPeriod))*stepsForPeriod;
		...
		return game.findLastIndex(g => g.includes('#')) + Math.floor((1000000000000-stepsBeforePeriod) / (stepsForPeriod)) * period;
	}
	...
	printGame(game: string[][]) {
		console.log();
		(JSON.parse(JSON.stringify(game)) as string[][]).reverse().forEach((g) => console.log(g.join('')));
		console.log();
	}
