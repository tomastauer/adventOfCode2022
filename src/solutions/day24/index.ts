...
export default class Day24 implements Solution {
	...
	solvePart2(input: string[]) {
		...
		const secondRun = this.dfs(
			{ time: firstRun, position: { x: finish.x, y: finish.y } },
			{ x: 1, y: 0 },
			emptyPositions,
		);
		const thirdRun = this.dfs(
			{
				time: secondRun,
				position: { x: start.position.x, y: start.position.y },
			},
			finish,
			emptyPositions,
		);
	...
	dfs(start: State, finish: Position, emptyPositions: Position[][]) {
		...
		while (states.length) {
			...
			if (
				state.position.x === finish.x && state.position.y === finish.y
			) {
			...
			if (
				e.find(({ x, y }) =>
					x === state.position.x - 1 && y === state.position.y
				)
			) {
				states.push({
					time: state.time + 1,
	...
	iterate(map: string[][][]) {
		const newMap: string[][][] = new Array(map.length).fill(0).map(() =>
			new Array(map[0].length).fill(0).map(() => [])
		);
