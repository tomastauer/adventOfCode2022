...
export default class Day18 implements Solution {
	solvePart1(input: string[]) {
		const space = new Array(25).fill(0).map(() =>
			new Array(25).fill(0).map(() => new Array(25).fill(0))
		);
	...
	solvePart2(input: string[]) {
		const space = new Array(23).fill(0).map(() =>
			new Array(23).fill(0).map(() => new Array(23).fill(0))
		);
	...
	traverse(current: Position, space: number[][][]) {
		...
		do {
			...
			visited.add(p.s);
			toVisit.push(
				...this.getNeighbors(p, space).filter((c) =>
					!visited.has(c.s) && !toVisit.find((f) => f.s === c.s) &&
					space[c.x][c.y][c.z] === 0
				),
			);
